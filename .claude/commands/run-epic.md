...
4. Respect Route per story: `Route=Human` stories are surfaced to the user rather
   than auto-run; everything else runs in this session. This repo sets
   `SUBSYSTEM_ROUTING=false`, so a story still marked `Route=Local` is stale
   metadata — run it as Claude, do not look for a local delegate.
