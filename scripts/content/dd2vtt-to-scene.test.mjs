...
import {
  ...
  parseArgs, convertFile, keysJournal, notesFromKeys,
...
test('keysJournal is GM-only and its page ids match the pins', () => {
  ...
  assert.ok(j.pages.every(p => p.ownership.default === 0), 'pages must be GM-only');
  const pins = notesFromKeys(KEYS, 100, rel);
  assert.deepEqual(j.pages.map(p => p._id), pins.map(p => p.pageId));
});
...
test('convertFile writes the keys journal and pins the scene from a spec', async () => {
  ...
  const { scene, journalOut } = await convertFile(ddPath, {
    background: 'DnD/x/m.png', out, gridDistance: 5,
    keys: specPath, keysJournal: keysJournalPath,
  });
  ...
  assert.deepEqual(journal.pages.map(p => p._id), scene.notes.map(n => n.pageId));
